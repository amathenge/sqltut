...
another_spongebob = session.scalars(select(User).where(User.name == 'Spongebob')).first()
print(another_spongebob)

session.add_all(
    [
        User('Tony', 'Tony Mwai'),
        User('Andrew', 'Andrew Mathenge'),
        User('Peter', 'Peter Wolf'),
        User('Julius', 'Julius Caesar')
    ]
)

result = session.scalars(select(User)).fetchall()
